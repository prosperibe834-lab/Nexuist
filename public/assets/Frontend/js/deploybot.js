...
// Populate deployed summary dynamic values (if present)
document.addEventListener('DOMContentLoaded', () => {
    try {
        const deployed = document.getElementById('deployedSummary');
        if (!deployed) return;
        const data = deployed.dataset.investment ? JSON.parse(deployed.dataset.investment) : null;
        if (!data) return;

        // Update progress fill if present. Be resilient if end_date is missing.
        const fill = document.getElementById('deployedProgressFill');
        if (fill && data.start_date) {
            ...
            if (data.end_date) {
                ...
                // fallback: use provided duration fields or 30 days
                const fallbackDays = (data.duration_days || data.investment_duration_days || 30);
                const total = Math.max(1, fallbackDays * 24 * 60 * 60 * 1000);
                const elapsed = Math.max(0, now - start);
                const pct = Math.min(100, Math.round((elapsed / total) * 100));
                fill.style.width = pct + '%';
            }
        ...
        // Optionally refresh numeric values if backend updated (simple client-side update)
        const investedEl = deployed.querySelector('.deployed-values .value');
        if (investedEl && data.investment_amount) {
            investedEl.innerText = '$' + Number(data.investment_amount).toFixed(2);
        }

    } catch (err) {
        // ignore parsing errors
        console.error('Deployed summary init error', err);
    }
});
...
document.addEventListener("DOMContentLoaded", () => {
    const trackingContainerNode = document.getElementById("deployedSummary");
    if (!trackingContainerNode) return;

    // Safely captures the parsed live Laravel dataset profile string context inside your front-end JS scope
    try {
        const structuralDataPayload = JSON.parse(trackingContainerNode.dataset.investment);
        if(structuralDataPayload) {
            console.log("Telemetry Payload Connected Matrix Session Active:", structuralDataPayload);
        }
    } catch (invalidJsonError) {
        console.warn("Telemetry Payload Node Data unavailable or unassigned.");
    }
});